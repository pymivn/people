...
`p` là một trỏ tới `x`, `x` có kiểu `int` nên `p` có kiểu `*int`. Còn giá trị của `p` là gì? `0xc000014110` là cách viết hệ hexadecimal (hệ 16), nó thường được dùng tới khi viết địa chỉ bộ nhớ máy tính.
