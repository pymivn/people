...
của nó. Tức int sẽ = 0, bool = false, string = "", pointer = nil. Nếu có, biểu thức theo sau sẽ được tính toán và biến sẽ chứa giá trị này.
...
cung cấp sẵn struct tương ứng giá trị JSON nói trên nếu dịch vụ đó có Go client
SDK.
...
Go cũng không khác nhiều với pointer trong C, nhưng sẽ là thứ rất khác biệt với người đến
từ Python.
...
`p` là một pointer tới `x`, `x` có kiểu `int` nên `p` có kiểu `*int`. Còn giá trị của `p` là gì? `0xc000014110` là cách viết hệ hexadecimal (hệ 16), nó thường được dùng tới khi viết địa chỉ bộ nhớ máy tính.
...
Còn có các tên goi khác như memory location, address.

Trong máy tính, bộ nhớ (RAM) sẽ lưu giữ các giá trị của chương trình
đang chạy, các thanh RAM sẽ có nhiều "ô", và mỗi ô sẽ được đánh số (trên thực tế phưc tạp hơn khi phân thành 2 loại physical address/logical address nhưng không bàn tới ở đây). Khi
tạo ra 1 giá trị trong Go, giá trị đó sẽ được chứa trong 1 ô nào đó,sử dụng phép toán [`&` (address operator)](https://golang.org/ref/spec#Address_operators) sẽ lấy được địa chỉ này là một con số nguyên dương biểu diễn ở dạng hex.

### Pointer là gì
Một pointer là một memory address - chỉ có vậy.
Nói `p` là một pointer point đến `x` là cách nói thuật ngữ của câu
`p` là giá trị của memory address chứa `x`.

Trong Go, một pointer luôn phải trỏ tới 1 giá trị x nào đó, ngoại trừ pointer mới declare sẽ có giá trị `nil`.

Nếu 1 tấm biến có ghi địa chỉ của 1 quán ăn, thì tấm biến chính là pointer tới quán ăn đó.
...
Vì pointer là memory location, tức một con số kiểu `int`, nó rất nhỏ, nhẹ. Người ta dùng nó vì muốn "nhẹ", muốn tiết kiệm bộ nhớ (RAM).
...
Việc này thực hiện bằng cách gọi function với pointer argument và return kiểu pointer. Khi call function với pointer argument, point argument vẫn được copy, nhưng copy address chỉ là copy 1 số int nhỏ bé.
...
Nếu nhìn kỹ ra, lý do dùng pointer nói trên là 1 cách để "tối ưu" về bộ nhớ và tốc độ, nhưng việc này lại không tôi ưu cho sự đơn giản, dễ hiểu của code - thứ luôn được đánh giá cao hơn trong các chương trình thực tế.

Sẽ không bao giờ bạn thấy function `sump` nào trông thế này cả, kể cả nó
có vẻ như dùng it tài nguyên hơn:
...
Nếu tới đây vẫn chưa hiểu pointer là gì, hãy thử vận may với bài viết của [Dave Cheny](https://dave.cheney.net/2017/04/26/understand-go-pointers-in-less-than-800-words-or-your-money-back)


## Method
method của struct là một thứ cú pháp rất sáng tạo và khác biệt vơi các ngôn ngữ phổ biến (C, Python)... nhưng nó chỉ là 1 cách viết đơn giản (syntactic sugar) để pass argument struct cho function (method) đó.
...
`struct` được mặc định gán vào cho method gọi là `method receiver`,
method receiver hầu hết đều là pointer, lý do xem bên trên.
`method receiver` giống như `self` argument trong method của Python class.

## Interface
Một biến kiểu `interface{}` có thể chứa mọi giá trị, biến Go thành dynamic typing giống như Python, nhưng đấy không phải lý do người ta tạo ra interface.
...
1 in {5: 'a', 6: 'b'}  # O(1)
```
chỉ thấy 1 chữ `in` và không thấy **rõ** là phải mất bao nhiêu phép tính, thì Go tối ưu thiết kế cho việc đó, dẫn tới việc muốn tìm 1 giá trị trong 1 slice/array, chỉ có cách là dùng 1 vòng for duyệt qua từng phần tử.
...
Go đơn giản - đơn giản tức là ít khái niệm, ít cú pháp. Nhưng không dễ - dễ tức là dễ làm 1 việc gì đó. Kiểm tra 1 phần tử trong slice, thêm 1 phần tử vào giữa array, ... các "bài toán nhỏ" mà Python chỉ mất 1 dòng code, thì trong Go là cả một... vấn đề lớn mà người ta phải viết hẳn [wiki rồi chia sẻ](https://github.com/golang/go/wiki/SliceTricks).
...
### Like
- Compile thành binary: deploy trở thành câu chuyện rất đơn giản
- Bộ stdlib rất đầy đủ: HTTP, JSON - hai công cụ rất quan trọng của DevOps.
...
## Kết luận
Go là một ngôn ngữ đơn giản nhưng không dễ, ai cũng học được nhưng để có thể dùng thoải mái sẽ cần một thời gian luyện tập cho quen sau khi đã dính lời nguyền ngắn gọn, dễ của Python. Go là một ngôn ngữ hiện đại, tốt, đáng bỏ thời gian đầu tư làm quen trong bộ công cụ của bạn... và vẫn yêu Python.
